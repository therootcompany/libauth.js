...
/**
 * @param {Boolean} failedValidation
 * @returns {AuthError}
 */
function CODE_REDEEMED(failedValidation = true) {
  if (false !== failedValidation) {
    throw DEVELOPER_ERROR(
      "'CODE_REDEEMED' must only be thrown when the code hasn't failed validation",
    );
  }
  return create(
    "That verification code has already been used successfully. You cannot use it again.",
    {
      status: 400,
 ...
 * @returns {AuthError}
 */
function CODE_INVALID(detail, failedValidation = true) {
  let status = 403;
  if (!failedValidation) {
    status = 400;
  }
