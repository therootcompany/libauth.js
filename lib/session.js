...
module.exports = function (issuer, PRIVATE_KEY, myOptions) {
  ...
  let opts = {
    ...
    _developmentSendSecretVerificationCode:
      myOptions._developmentSendSecretVerificationCode,
    // assigned elsewhere
    secret: "",
    logout:
      /**
  ...
  if (PRIVATE_KEY && !keypair.private) {
    try {
      //@ts-ignore
      PRIVATE_KEY = FsSync.readFileSync(PRIVATE_KEY, "utf8").trim();
      console.warn(
  ...
  async function _strategyHandler(req, res) {
    ...
    let allClaims = await __strategyHandler(req, res);
    if (!allClaims) {
      return;
    }

    // TODO deprecate?
    let { claims, id_claims, access_claims, refresh_claims } = allClaims;
    if (!claims) {
      claims = {};
    }
    // TODO move this to where claims are set up?
  ...
  function router() {
    ...
    if (opts._byChallenge) {
      app.post("/challenge/order", a3k._challengeRoutes.orderVerification);
      app.get("/challenge/status", a3k._challengeRoutes.checkStatus);
      app.get("/challenge", a3k._challengeRoutes.checkStatus);
      app.post("/challenge/finalize", a3k._challengeRoutes.redeemCode);
      //app.post("/challenge/redeem", a3k._challengeRoutes.redeemCode);
      app.post("/challenge/exchange", a3k._challengeRoutes.exchangeReceipt);
      //app.use("/challenge/challenge", byChallenge);
      //
    }

    if (opts._byCredentials) {
      app.post("/session", a3k._credentialRoutes);
    }

    if (a3k._oidcRoutes["accounts.google.com"]) {
      app.post(
        "/session/oidc/accounts.google.com",
        a3k._oidcRoutes["accounts.google.com"],
      );
      // deprecated
      app.post(
    ...
    if (opts._forExchange) {
      app.post("/exchange", a3k._exchangeRoutes);
    }

    if (opts._forRefresh) {
      app.post("/refresh", a3k._refreshRoutes);
    }

    if (!a3k._logoutRoutes) {
      a3k.logout(function () {});
    }
    app.delete("/session", a3k._logoutRoutes);
  ...
  let a3k = {
    ...
    challenge: function (_opts) {
      ...
      a3k._challengeRoutes = require("./magic-link.js").createRouter({
        iss: opts.issuer,
        verifier: opts.verifier,
        DEVELOPMENT: opts.DEVELOPMENT,
        _developmentSendSecretVerificationCode:
          opts._developmentSendSecretVerificationCode,
        //@ts-ignore
    ...
    credentials: function (_opts) {
      ...
      a3k._credentialRoutes = async function (req, res, next) {
        let creds;
      ...
      opts._byCredentials = true;
      return a3k._credentialRoutes;
    },
    ...
    refresh: function (_refreshClaims) {
      ...
      /** @type {import('express').Handler} */
      a3k._refreshRoutes = async function (req, res, next) {
      ...
      opts._forRefresh = true;
      return a3k._refreshRoutes;
    },
    ...
    exchange: function (_exchangeClaims) {
      ...
      /** @type {import('express').Handler} */
      a3k._exchangeRoutes = async function (req, res) {
    ...
    logout: function (_logout) {
      ...
      /** @type {import('express').Handler} */
      a3k._logoutRoutes = async function (req, res, next) {
      ...
      opts._forLogout = true;
      return a3k._logoutRoutes;
    },
    /** @param {any} _opts // TODO */
    oauth2: function (_opts) {
      ...
      if (_gh?.clientSecret) {
        ...
        a3k._oauth2Routes["github.com"] = ghRoutes;
      }

      return a3k._oauth2Routes;
    },
    ...
    oidc: function (_opts) {
      ...
      if (_goog?.clientId) {
        ...
        /// @ts-ignore
        a3k._oidcRoutes[_issuerName] = googRoutes;
      }

      return a3k._oidcRoutes;
    },
    options:
      /**
       * @param {any} _opts // TODO
    ...
    // TODO /** @type {Record<string, import('express').Handler>} */
    _challengeRoutes: {
      /** @type {import('express').Handler} */
      ...
      redeemCode: function () {},
      /** @type {import('express').Handler} */
      exchangeReceipt: function () {},
    },
    _oidcRoutes: {
      /** @type {import('express').Handler} */
      "accounts.google.com": function () {},
    },
    _oauth2Routes: {
      /** @type {import('express').Handler} */
      "github.com": function () {},
    ...
    /** @type {import('express').Handler} */
    _credentialRoutes: function () {},
    /** @type {import('express').Handler} */
    _exchangeRoutes: function () {},
    /** @type {import('express').Handler} */
    _refreshRoutes: function () {},
    /** @type {import('express').Handler} */
    _logoutRoutes: function () {},
