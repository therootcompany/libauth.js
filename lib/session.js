...
LibAuth.create = function (issuer, PRIVATE_KEY, myOptions) {
  ...
  */
  ...
  function _oidc(pluginOpts) {
    ...
    /** @type {import('express').Handler} */
    async function authorizationRedirect(req, res) {
    ...
    function exchangeToken(req, res, next) {
      async function _oidcVerify() {
        let jwt = (req.headers.authorization || "").replace(/^Bearer /, "");
        let jws = await _oidcVerifyToken(jwt, oidcOpts);

        //@ts-ignore
        req.__libauth.jws(jws);
      ...
      //@ts-ignore
      return Promise.resolve().then(_oidcVerify).catch(next);
    }
  ...
  /** @param {any} _opts // TODO */
  //@ts-ignore
  libauth.challenge = require("./lib/magic.js").create(libauth, opts);
