...
// TODO papercut keypair vs privateKey
module.exports = function (
  HMAC_SECRET,
  { keypair, notify, store, iss, getClaims, googleClientId }
) {
  ...
  async function _getClaims(opts) {
  ...
  if (DEVELOPMENT) {
    console.info("ENV=DEVELOPMENT:", DEVELOPMENT);
  ...
  async function setAuthnRefresher(res, allClaims) {
    let refresh_token;
    let maxAge;
  ...
  async function grantTokensAndCookie(allClaims, res) {
    let { id_token, access_token } = await grantTokens(allClaims);
    await setAuthnRefresher(res, allClaims);
    return { id_token, access_token };
  }
  ...
  let byOidc = async function (req, res) {
    // TODO session.byOidc(req);
    let allClaims = await _getClaims({
      method2: "oidc",
    ...
    return grantTokensAndCookie(allClaims, res);
  };
  let byCredentials = async function (req, res) {
    // TODO session.byCredentials(req);
    let allClaims = await _getClaims({
      method2: "credentials",
      claims2: req.body,
    });
  ...
  app.use("/api/authn/", cookieParser(HMAC_SECRET)); // needed to set cookies?
  if (googleClientId) {
    app.post(
      "/api/authn/session/oidc/google.com",
      verifyGoogleToken(googleClientId, googleVerifierOpts),
      byOidc
    );
  }
  app.post("/api/authn/session", byCredentials);
  app.use("/", byChallenge); // /api/authn/challenge
  app.post("/api/authn/exchange", byIdToken);
  app.post("/api/authn/refresh", byCookie); // cookies only parsed here
  app.delete("/api/authn/session", logout);
