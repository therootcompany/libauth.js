  ...
  function router() {
    ...
    if (process.env.GITHUB_CLIENT_SECRET) {
      app.get("/webhooks/oauth2/github", async function (req, res) {
        ...
        //var url = new URL(`${issuer}?${resp.toJSON().body}`);
        var url = new URL(`${issuer}#${resp.toJSON().body}`);
        var token = url.searchParams.get("access_token");
        url.searchParams.set("issuer", "github.com");
      ...
      let byOauth2 = async function (req, res) {
        req[opts.authnParam] = {
          strategy: "oauth2",
    ...
    oidc: function (_opts) {
      ...
      if (opts.DEVELOPMENT && !opts.__DEVELOPMENT_2) {
        ...
        if (opts.oidc?.google?.clientId) {
          console.info(
            "[auth3000] [ENV=DEVELOPMENT] Allow Expired Google Tokens"
          );
