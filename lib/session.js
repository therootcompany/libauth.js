...
async function parsePrivateKey(PRIVATE_KEY, keypair) {
  ...
  if (!keypair.public.kid) {
    ...
    let thumb = await JWT.thumbprint(keypair.private).catch(function (err) {
...
LibAuth.create = function (issuer, PRIVATE_KEY, myOptions) {
  ...
  // gets public key sync, and thumbprint async
  void parsePrivateKey(PRIVATE_KEY, keypair);
  ...
  async function _issueToken(claims) {
    ...
    return await JWT.sign(keypair.private, {
      iss: libOpts.issuer,
      exp: exp,
      claims: claims,
    });
