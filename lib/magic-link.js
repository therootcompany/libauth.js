...
module.exports = function ({
  ...
  app.post("/api/authn/challenge/issue", async function (req, res) {
    ...
    await store.set(
      ...
      Object.assign({}, claims, {
        ...
        ordered_ip: res.socket.remoteAddress,
      })
    );
  ...
  app.get("/api/authn/challenge", async function (req, res) {
    ...
    if (!meta || !mlIsFresh(meta)) {
      throw E.INVALID_TOKEN();
    }

    res.json({
      success: true,
      id: id,
      state: meta.verified_by ? "valid" : "pending",
  ...
  app.post("/api/authn/challenge/complete", async function (req, res) {
    ...
    if (!id) {
      // TODO maybe use separate id so we can track attempts
      // https://example.com/#?token=1234abcd.efgh0987
      //id = secretToId(HMAC_SECRET, secret, mlIdBytes, mlIdEncoding);
    }
    ...
    // TODO session.byIdentifierChallenge(req);
    let x = await _grantTokensAndCookie(allClaims, res);
    let { id_token, access_token } = x;
