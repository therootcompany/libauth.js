 ...
 */
Challenger.create = function (opts) {
  let app = require("@root/async-router").Router();
  let routes = Challenger._create(opts);
...
Challenger._create = function ({
  ...
  let orderVerification = async function (req, res) {
    ...
    let { code, receipt, id } = _rndCode(4, "hex");
    ...
    let expiration = new Date(d.valueOf() + parseDuration(maxAge));
    let result = {
      success: true,
      id: id,
      receipt: receipt,
   ...
   */
  function _rndCode(bytes, enc) {
    let code = rnd(bytes, enc);
    let receipt = Challenger._hashify(
      HMAC_SECRET,
      code,
      receiptByteCount,
      receiptEncoding
    );
    let id = Challenger._hashify(HMAC_SECRET, receipt, idByteCount, idEncoding);
  ...
  let checkStatus = async function (req, res) {
    ...
    if (!meta || !Challenger._isFresh(meta, maxAgeMs, maxAttempts)) {
      throw E.CODE_INVALID();
    }

    let creation = new Date(meta.ordered_at);
    let expiration = new Date(creation.valueOf() + parseDuration(maxAge));
    res.json({
      success: true,
      id: id,
  ...
  async function _attemptCounter(id, code, req, res) {
    ...
    if (!Challenger._codesMatch(meta.code, code)) {
 ...
 */
Challenger._hashify = function (HMAC_SECRET, code, bytes, enc) {
  if (!code) {
 ...
 * @param {string} b
 */
Challenger._codesMatch = function (a, b) {
 ...
 */
Challenger._isFresh = function (meta, maxAgeMs, maxAttempts) {
  if (meta.deleted_at || meta.exchanged_at || meta.attempts >= maxAttempts) {
    return false;
  }
  let now = Date.now();
